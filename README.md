...
- 종료는 `Ctrl-C`.
- ⚠️ **코드를 새로 받으면(`git pull`) 백엔드를 반드시 재시작**(`Ctrl-C` 후 `python run.py` 다시). `run.py`는 uvicorn 을 `--reload` 없이 띄우므로, 떠 있는 프로세스는 옛 코드를 그대로 들고 있어 파일만 바꿔선 반영되지 않는다. (단일포트 모드면 `frontend/dist`도 다시 빌드해야 프론트 변경 반영.)
